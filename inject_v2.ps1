...
# List of new translations to ensure are present
$extraTranslations = @'
  ...
  'Name':                           { English: 'Name',                           Hindi: 'नाम',                         Marathi: 'नाव',                         Urdu: 'नाम' },
  'Abbr':                           { English: 'Abbr',                           Hindi: 'संक्षिप्त नाम',               Marathi: 'संक्षिप्त नाव',               Urdu: 'مخفف' },
  'Detailed Analysis — Click to Explore': { English: 'Detailed Analysis — Click to Explore', Hindi: 'विस्तृत विश्लेषण — तलाशने के लिए क्लिक करें', Marathi: 'तपशीलवार विश्लेषण — शोधण्यासाठी क्लिक करा', Urdu: 'تفصیلی تجزیہ — دریافت کرنے के لیے کلک کریں' },
...
foreach ($line in $lines) {
    if ($line -match "^\s*'([^']+)'\s*:") {
        ...
        if ($seenKeys.ContainsKey($key)) {
            continue # Duplicate key removal
        }
        $seenKeys[$key] = $true
    }
    ...
    if ($line -match "'Node':" -and -not $inserted) {
        $newLines += $extraTranslations -split "`n"
        $inserted = $true
    }
}
